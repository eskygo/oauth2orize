...
/**
 * Registers a function used to serialize session
 *
 * Examples:
 *
 *  server.serializeSession = function(obj, req, options, done) {
 *    if (!req.session) { return done('OAuth2orize requires session support. Did you forget app.use(express.session(...))?'); }
 *    var key = options.sessionKey || "authorize";
 *    if (!req.session[key]) { return done('Unable to load OAuth 2.0 transactions from session'); }
 *    var uid = someuidgen();
 *    req.session[key][uid] = obj;
 *    done(null, uid);
 *  });
 *
 * @api public
 */
Server.prototype.serializeSession = function(obj, req, options, done) {
    options = options || {};
   if (!req.session) { return done(new Error('OAuth2orize requires session support. Did you forget app.use(express.session(...))?')); }
   var key = options.sessionKey || "authorize";
   req.session[key] = req.session[key] || {};
   var uid = require('./utils.js').uid(options.idLength || 8);
   req.session[key][uid] = obj;
   done(null, uid);
}

/**
 * Registers a function used to deserialize session
 *
 * Examples:
 *
 *     server.serializeSession = function(obj, req, options, done) {
 ...
 *       var key = options.sessionKey || "authorize";
 *       if (!req.session[key]) { return done('Unable to load OAuth 2.0 transactions from session'); }
 *       done ( null, req.session[key][id])
 *     });
 *
 * @api public
 */
Server.prototype.deserializeSession = function(id, req, options, done) {
   if (!req.session) { return done(new Error('OAuth2orize requires session support. Did you forget app.use(express.session(...))?')); }
   var key = options.sessionKey || "authorize";
   if (!req.session[key]) { return done(new ForbiddenError('Unable to load OAuth 2.0 transactions from session')); }
   done ( null, req.session[key][id])
}


/**
 * Registers a function used to remove session
 *
 * Examples:
 *
 *     server.serializeSession = function(obj, req, options, done) {
 ...
 *       var key = options.sessionKey || "authorize";
 *       if (!req.session[key]) { return done('Unable to load OAuth 2.0 transactions from session'); }
 *       delete req.session[key][id];
 *       done ( null )
 *     });
 *
 * @api public
 */
Server.prototype.removeSession = function(id, req, options, done) {
   if (!req.session) { return done(new Error('OAuth2orize requires session support. Did you forget app.use(express.session(...))?')); }
   var key = options.sessionKey || "authorize";
   if (!req.session[key]) { return done(new ForbiddenError('Unable to load OAuth 2.0 transactions from session')); }
   delete req.session[key][id];
